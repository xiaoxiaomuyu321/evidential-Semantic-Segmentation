...
from evidence_semantic_segmentation.data_loader.MuSeg import create_dataloader
...
def train_one_epoch(model, loader, criterion, optimizer, device, num_classes, scaler, use_amp, epoch, epoches):
    ...
    for images, labels in pbar:
        ...
        # 更新进度条内容
        pbar.set_postfix({'total_loss': f"{current_avg_loss:.4f}",
                          'time': f"{elapsed:.1f}s"})
    aver_loss = current_avg_loss
    ...
    return aver_loss, acc, acc_cls, mean_iu, fwavacc
...
def validate_one_epoch(model, loader, criterion, device, num_classes, use_amp, epoch, epoches):
    ...
    acc, acc_cls, mean_iu, fwavacc = label_accuracy_score(true_np, pred_np, num_classes)
    if epoch ==epoches:
        tqdm.write(f"✅ Final val | Loss: {aver_loss:.4f} | acc: {acc:.4f} | acc_cls: {acc_cls:.4f} | mIoU: {mean_iu:.4f} | fwavacc: {fwavacc:.4f}")
...
def train(model, train_loader, val_loader, criterion, optimizer, device, args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # 验证条件
        if epoch >= 280 and epoch % 1 == 0 or epoch % 300 == 0:
...
def main():
    ...
    # 参数定义
    parser.add_argument('--data_root', type=str, default='datasets/museg_voc')
    parser.add_argument('--project_name', type=str, default='museg_voc')
    ...
    criterion = SegLoss(num_classes=args.num_classes, loss_type="ce_dice", cls_weights=None,
                        annealing_step=100, ignore_index=None).to(device)
